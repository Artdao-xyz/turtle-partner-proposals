...
export default function Button({ children, href, target, className = "" }: ButtonProps) {
    return (
        <a href={href} target={target} className={`relative p-[1.5px] rounded-[100px] shadow-green-turtle ${className} cursor-pointer group overflow-hidden w-auto md:w-auto`}>
            {/* Base gradient */}
            <div className="absolute inset-0 bg-gradient-to-b from-green-turtle to-background rounded-[100px]"></div>
            {/* Hover gradient */}
            <div className="absolute inset-0 bg-gradient-to-t from-green-turtle to-background rounded-[100px] opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-in-out"></div>
