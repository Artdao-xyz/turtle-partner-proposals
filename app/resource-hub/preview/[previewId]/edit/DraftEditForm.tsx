...
export default function DraftEditForm({
  ...
  const [heroImage] = useState(preservedFrontmatter.heroImage ?? "");
  ...
  function buildFullContent(): string {
    const raw: Record<string, unknown> = {
      slug: preservedFrontmatter.slug ?? "",
      title,
      subtitle,
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/article-upload/draft", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ previewId, content: buildFullContent() }),
      });
